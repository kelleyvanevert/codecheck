...
function add (a, b) {
  if (typeof a === "number") {
    ...
    return Array
      .from({ length: Math.max(a.length, b.length) })
      .map((__, i) => (a[i] || 0) + (b[i] || 0));
  } else {
    return _.union(Object.keys(a), Object.keys(b))
      .reduce((sum, key) => {
...
function count (node) {
  ...
  // recurse on children
  const sum = arr
    .map(key => node[key].map(count).reduce(add, {}))
    .concat(direct.map(key => count(node[key])))
    .reduce(add, {})
...
const countAs = Object.entries(countTypes).reduce((countAs, [sumKey, nodeTests]) => {
  nodeTests.filter(test => typeof test === "string").forEach(type => {
...
const ESTreeStructure = {
  ...
  "ConditionalExpression": { arr: [], direct: ["test", "alternate", "consequent"] },
